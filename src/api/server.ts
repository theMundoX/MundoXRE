...
function title(){return ({overview:'Market Pulse Report',listings:'Filtered Opportunity List',priceDrops:'Price Drop Proof List: Before / After Changes',creativeFinance:'Creative Finance Report',delisted:'Delisted / Failed Listing Report',multifamily:'Multifamily Report',zipCodes:'ZIP Code Rankings',dataQuality:'Data Quality Report',changeFeed:'Change Feed'})[view]||'Report'}
function subtitle(){if(view==='priceDrops')return fmt((analytics.reports.priceDrops||[]).length)+' price-drop rows loaded. Average cards above are calculated from these before/after records.';return view==='listings'?fmt(listings.total)+' matching active listings':'Generated '+new Date(analytics.generated_at).toLocaleString()}
