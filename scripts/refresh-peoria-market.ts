...
async function main() {
  ...
  const steps: Step[] = [
    ...
    },
    {
      ...
      command: ["npx", "tsx", "scripts/ingest-listings-fast.ts", "--state", "IL", "--zips", PEORIA_ZIPS, "--concurrency", dryLimit("3", "1"), ...(DRY_RUN ? ["--dry-run", "--skip-match", "--allow-partial"] : ["--skip-match", "--allow-partial"])],
      required: false,
      supportsDryRun: true,
      skip: SKIP_LISTINGS,
