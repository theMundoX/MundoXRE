#!/usr/bin/env tsx
/**
 * Repeatable Cleveland, OH market coverage refresh.
 *
 * Cleveland uses Cuyahoga County public parcels plus shared listing, contact,
...
async function main() {
  ...
  const resultPath = join(logDir, `cleveland-oh-${stamp}.json`);
  const results: StepResult[] = [];

  await mkdir(logDir, { recursive: true });

  console.log("MXRE Cleveland, OH market refresh");
  ...
  const steps: Step[] = [
    {
      name: "Ingest Cuyahoga County public parcels",
      command: ["npx", "tsx", "scripts/ingest-cuyahoga-oh.ts"],
      required: true,
    ...
    {
      name: "Refresh Cleveland Redfin listing signals",
      command: ["npx", "tsx", "scripts/ingest-listings-fast.ts", "--state", "OH", "--county", "Cuyahoga", "--concurrency", dryLimit("3", "1"), ...(DRY_RUN ? ["--dry-run", "--skip-match", "--allow-partial"] : ["--skip-match", "--allow-partial"])],
      required: false,
      supportsDryRun: true,
      skip: SKIP_LISTINGS,
    ...
    {
      name: "Link Cleveland active listings",
      command: ["npx", "tsx", "scripts/link-market-listings-fast.ts", "--state=OH", "--city=CLEVELAND", `--county_id=${COUNTY_ID}`, "--create-shells", ...(DRY_RUN ? ["--dry-run"] : [])],
      required: false,
      supportsDryRun: true,
      skip: SKIP_LISTINGS,
    ...
    {
      name: "Enrich Cleveland Redfin listing detail pages",
      command: ["npx", "tsx", "scripts/enrich-redfin-detail-pages.ts", "--state=OH", "--city=CLEVELAND", `--limit=${dryLimit("5000", "25")}`, `--delay-ms=${dryLimit("300", "100")}`, ...(DRY_RUN ? ["--dry-run"] : [])],
      required: false,
      supportsDryRun: true,
      skip: SKIP_LISTINGS,
    ...
    {
      name: "Classify Cleveland active parcel assets",
      command: ["npx", "tsx", "scripts/classify-market-assets.ts", "--state=OH", "--city=CLEVELAND", `--county_id=${COUNTY_ID}`, "--active-listings-only", `--batch-size=${dryLimit("1500", "250")}`, ...(DRY_RUN ? ["--dry-run"] : [])],
      required: false,
      supportsDryRun: true,
      skip: SKIP_CLASSIFY,
      timeoutMs: DRY_RUN ? 5 * 60_000 : 20 * 60_000,
    ...
    {
      name: "Normalize Cleveland listing agent contacts",
      command: ["npx", "tsx", "scripts/enrich-listing-agent-contacts.ts", "--state=OH", "--city=CLEVELAND", `--limit=${dryLimit("5000", "25")}`, ...(DRY_RUN ? ["--dry-run"] : [])],
      required: false,
      supportsDryRun: true,
      skip: SKIP_AGENT_CONTACTS,
    },
    {
      name: "Score Cleveland creative finance signals",
      command: ["npx", "tsx", "scripts/score-creative-finance-signals.ts", "--state=OH", "--city=CLEVELAND", `--limit=${dryLimit("5000", "25")}`, ...(DRY_RUN ? ["--dry-run"] : [])],
      required: false,
      supportsDryRun: true,
      skip: SKIP_CREATIVE,
    },
    {
      name: "Paid Cleveland Property Detail enrichment",
      command: [
    ...
    {
      name: "Audit Cleveland agent coverage",
      command: ["npx", "tsx", "scripts/audit-on-market-agent-coverage.ts", "--state=OH", "--city=CLEVELAND"],
      required: false,
      supportsDryRun: true,
      skip: SKIP_AUDITS,
    },
    {
      name: "Audit Cleveland readiness",
      command: ["npx", "tsx", "scripts/market-readiness-summary.ts", "--state=OH", "--city=CLEVELAND", `--county_id=${COUNTY_ID}`],
      required: false,
      supportsDryRun: true,
      skip: SKIP_AUDITS,
    },
  ...
  console.log("\nCleveland market refresh summary");
  console.log("=".repeat(48));
...
main().catch((error) => {
  console.error("Fatal Cleveland market refresh error:", error instanceof Error ? error.message : error);
  process.exit(1);
});
