#!/usr/bin/env tsx
/**
 * Repeatable South Bend, IN market coverage refresh.
 *
 * South Bend uses St. Joseph County public parcels plus shared listing, contact,
...
const COUNTY_ID = 797737;
const SOUTH_BEND_ZIPS = [
  "46601", "46613", "46614", "46615", "46616", "46617", "46619", "46628",
  "46635", "46637",
].join(",");
...
async function main() {
  ...
  const resultPath = join(logDir, `south-bend-in-${stamp}.json`);
  const results: StepResult[] = [];

  await mkdir(logDir, { recursive: true });

  console.log("MXRE South Bend, IN market refresh");
  ...
  const steps: Step[] = [
    {
      name: "Ingest St. Joseph County public parcels",
      command: ["npx", "tsx", "scripts/ingest-in-parcels.ts", "--county=ST. JOSEPH"],
      required: true,
    ...
    {
      name: "Link South Bend active listings",
      command: ["npx", "tsx", "scripts/link-market-listings-fast.ts", "--state=IN", "--city=SOUTH BEND", `--county_id=${COUNTY_ID}`, "--create-shells", ...(DRY_RUN ? ["--dry-run"] : [])],
      required: false,
      supportsDryRun: true,
      skip: SKIP_LISTINGS,
    ...
    {
      name: "Enrich South Bend Redfin listing detail pages",
      command: ["npx", "tsx", "scripts/enrich-redfin-detail-pages.ts", "--state=IN", "--city=SOUTH BEND", `--limit=${dryLimit("5000", "25")}`, `--delay-ms=${dryLimit("300", "100")}`, ...(DRY_RUN ? ["--dry-run"] : [])],
      required: false,
      supportsDryRun: true,
      skip: SKIP_LISTINGS,
    ...
    {
      name: "Classify South Bend active parcel assets",
      command: ["npx", "tsx", "scripts/classify-market-assets.ts", "--state=IN", "--city=SOUTH BEND", `--county_id=${COUNTY_ID}`, "--active-listings-only", `--batch-size=${dryLimit("1500", "250")}`, ...(DRY_RUN ? ["--dry-run"] : [])],
      required: false,
      supportsDryRun: true,
      skip: SKIP_CLASSIFY,
      timeoutMs: DRY_RUN ? 5 * 60_000 : 20 * 60_000,
    ...
    {
      name: "Normalize South Bend listing agent contacts",
      command: ["npx", "tsx", "scripts/enrich-listing-agent-contacts.ts", "--state=IN", "--city=SOUTH BEND", `--limit=${dryLimit("5000", "25")}`, ...(DRY_RUN ? ["--dry-run"] : [])],
      required: false,
      supportsDryRun: true,
      skip: SKIP_AGENT_CONTACTS,
    },
    {
      name: "Score South Bend creative finance signals",
      command: ["npx", "tsx", "scripts/score-creative-finance-signals.ts", "--state=IN", "--city=SOUTH BEND", `--limit=${dryLimit("5000", "25")}`, ...(DRY_RUN ? ["--dry-run"] : [])],
      required: false,
      supportsDryRun: true,
      skip: SKIP_CREATIVE,
    },
    {
      name: "Paid South Bend Property Detail enrichment",
      command: [
    ...
    {
      name: "Audit South Bend agent coverage",
      command: ["npx", "tsx", "scripts/audit-on-market-agent-coverage.ts", "--state=IN", "--city=SOUTH BEND"],
      required: false,
      supportsDryRun: true,
      skip: SKIP_AUDITS,
    },
    {
      name: "Audit South Bend readiness",
      command: ["npx", "tsx", "scripts/market-readiness-summary.ts", "--state=IN", "--city=SOUTH BEND", `--county_id=${COUNTY_ID}`],
      required: false,
      supportsDryRun: true,
      skip: SKIP_AUDITS,
    },
  ...
  console.log("\nSouth Bend market refresh summary");
  console.log("=".repeat(48));
