#!/usr/bin/env tsx
/**
 * Repeatable Memphis, TN market coverage refresh.
 *
 * Memphis uses Shelby County public parcels plus shared listing, contact,
...
const COUNTY_ID = 1741244;
const MEMPHIS_ZIPS = [
  "38103", "38104", "38105", "38106", "38107", "38108", "38109", "38111",
  "38112", "38114", "38115", "38116", "38117", "38118", "38119", "38120",
  "38122", "38125", "38126", "38127", "38128", "38131", "38132", "38133",
  "38134", "38135", "38138", "38139", "38141", "38152",
].join(",");
...
async function main() {
  ...
  const resultPath = join(logDir, `memphis-tn-${stamp}.json`);
  const results: StepResult[] = [];

  await mkdir(logDir, { recursive: true });

  console.log("MXRE Memphis, TN market refresh");
  ...
  const steps: Step[] = [
    {
      name: "Ingest Shelby County public parcels",
      command: ["npx", "tsx", "scripts/ingest-shelby-tn.ts"],
      required: true,
    ...
    {
      name: "Refresh Memphis Redfin listing signals",
      command: ["npx", "tsx", "scripts/ingest-listings-fast.ts", "--state", "TN", "--zips", MEMPHIS_ZIPS, "--concurrency", dryLimit("3", "1"), ...(DRY_RUN ? ["--dry-run", "--skip-match", "--allow-partial"] : ["--skip-match", "--allow-partial"])],
      required: false,
      supportsDryRun: true,
      skip: SKIP_LISTINGS,
    ...
    {
      name: "Link Memphis active listings",
      command: ["npx", "tsx", "scripts/link-market-listings-fast.ts", "--state=TN", "--city=MEMPHIS", `--county_id=${COUNTY_ID}`, "--create-shells", ...(DRY_RUN ? ["--dry-run"] : [])],
      required: false,
      supportsDryRun: true,
      skip: SKIP_LISTINGS,
    ...
    {
      name: "Enrich Memphis Redfin listing detail pages",
      command: ["npx", "tsx", "scripts/enrich-redfin-detail-pages.ts", "--state=TN", "--city=MEMPHIS", `--limit=${dryLimit("5000", "25")}`, `--delay-ms=${dryLimit("300", "100")}`, ...(DRY_RUN ? ["--dry-run"] : [])],
      required: false,
      supportsDryRun: true,
      skip: SKIP_LISTINGS,
    ...
    {
      name: "Classify Memphis active parcel assets",
      command: ["npx", "tsx", "scripts/classify-market-assets.ts", "--state=TN", "--city=MEMPHIS", `--county_id=${COUNTY_ID}`, "--active-listings-only", `--batch-size=${dryLimit("1500", "250")}`, ...(DRY_RUN ? ["--dry-run"] : [])],
      required: false,
      supportsDryRun: true,
      skip: SKIP_CLASSIFY,
      timeoutMs: DRY_RUN ? 5 * 60_000 : 20 * 60_000,
    ...
    {
      name: "Normalize Memphis listing agent contacts",
      command: ["npx", "tsx", "scripts/enrich-listing-agent-contacts.ts", "--state=TN", "--city=MEMPHIS", `--limit=${dryLimit("5000", "25")}`, ...(DRY_RUN ? ["--dry-run"] : [])],
      required: false,
      supportsDryRun: true,
      skip: SKIP_AGENT_CONTACTS,
    },
    {
      name: "Score Memphis creative finance signals",
      command: ["npx", "tsx", "scripts/score-creative-finance-signals.ts", "--state=TN", "--city=MEMPHIS", `--limit=${dryLimit("5000", "25")}`, ...(DRY_RUN ? ["--dry-run"] : [])],
      required: false,
      supportsDryRun: true,
      skip: SKIP_CREATIVE,
    },
    {
      name: "Paid Memphis Property Detail enrichment",
      command: [
    ...
    {
      name: "Audit Memphis agent coverage",
      command: ["npx", "tsx", "scripts/audit-on-market-agent-coverage.ts", "--state=TN", "--city=MEMPHIS"],
      required: false,
      supportsDryRun: true,
      skip: SKIP_AUDITS,
    },
    {
      name: "Audit Memphis readiness",
      command: ["npx", "tsx", "scripts/market-readiness-summary.ts", "--state=TN", "--city=MEMPHIS", `--county_id=${COUNTY_ID}`],
      required: false,
      supportsDryRun: true,
      skip: SKIP_AUDITS,
    },
  ...
  console.log("\nMemphis market refresh summary");
  console.log("=".repeat(48));
...
main().catch((error) => {
  console.error("Fatal Memphis market refresh error:", error instanceof Error ? error.message : error);
  process.exit(1);
});
