#!/usr/bin/env tsx
/**
 * Repeatable Toledo, OH market coverage refresh.
 *
 * Toledo uses Lucas County public parcels plus shared listing, contact,
...
async function main() {
  ...
  const resultPath = join(logDir, `toledo-oh-${stamp}.json`);
  const results: StepResult[] = [];

  await mkdir(logDir, { recursive: true });

  console.log("MXRE Toledo, OH market refresh");
  ...
  const steps: Step[] = [
    {
      name: "Ingest Lucas County public parcels",
      command: ["npx", "tsx", "scripts/ingest-lucas-oh.ts", "--city=TOLEDO", ...(DRY_RUN ? ["--max-pages=1"] : [])],
      required: true,
    ...
    {
      name: "Refresh Toledo Redfin listing signals",
      command: ["npx", "tsx", "scripts/ingest-listings-fast.ts", "--state", "OH", "--county", "Lucas", "--concurrency", dryLimit("3", "1"), ...(DRY_RUN ? ["--dry-run", "--skip-match"] : ["--skip-match"])],
      required: false,
      supportsDryRun: true,
      skip: SKIP_LISTINGS,
    ...
    {
      name: "Link Toledo active listings",
      command: ["npx", "tsx", "scripts/link-market-listings-fast.ts", "--state=OH", "--city=TOLEDO", `--county_id=${COUNTY_ID}`, "--create-shells", ...(DRY_RUN ? ["--dry-run"] : [])],
      required: false,
      supportsDryRun: true,
      skip: SKIP_LISTINGS,
    ...
    {
      name: "Enrich Toledo Redfin listing detail pages",
      command: ["npx", "tsx", "scripts/enrich-redfin-detail-pages.ts", "--state=OH", "--city=TOLEDO", `--limit=${dryLimit("5000", "25")}`, `--delay-ms=${dryLimit("300", "100")}`, ...(DRY_RUN ? ["--dry-run"] : [])],
      required: false,
      supportsDryRun: true,
      skip: SKIP_LISTINGS,
    ...
    {
      name: "Classify Toledo active parcel assets",
      command: ["npx", "tsx", "scripts/classify-market-assets.ts", "--state=OH", "--city=TOLEDO", `--county_id=${COUNTY_ID}`, "--active-listings-only", `--batch-size=${dryLimit("1500", "250")}`, ...(DRY_RUN ? ["--dry-run"] : [])],
      required: false,
      supportsDryRun: true,
      skip: SKIP_CLASSIFY,
      timeoutMs: DRY_RUN ? 5 * 60_000 : 20 * 60_000,
    ...
    {
      name: "Normalize Toledo listing agent contacts",
      command: ["npx", "tsx", "scripts/enrich-listing-agent-contacts.ts", "--state=OH", "--city=TOLEDO", `--limit=${dryLimit("5000", "25")}`, ...(DRY_RUN ? ["--dry-run"] : [])],
      required: false,
      supportsDryRun: true,
      skip: SKIP_AGENT_CONTACTS,
    },
    {
      name: "Score Toledo creative finance signals",
      command: ["npx", "tsx", "scripts/score-creative-finance-signals.ts", "--state=OH", "--city=TOLEDO", `--limit=${dryLimit("5000", "25")}`, ...(DRY_RUN ? ["--dry-run"] : [])],
      required: false,
      supportsDryRun: true,
      skip: SKIP_CREATIVE,
    },
    {
      name: "Paid Toledo Property Detail enrichment",
      command: [
    ...
    {
      name: "Audit Toledo agent coverage",
      command: ["npx", "tsx", "scripts/audit-on-market-agent-coverage.ts", "--state=OH", "--city=TOLEDO"],
      required: false,
      supportsDryRun: true,
      skip: SKIP_AUDITS,
    },
    {
      name: "Audit Toledo readiness",
      command: ["npx", "tsx", "scripts/market-readiness-summary.ts", "--state=OH", "--city=TOLEDO", `--county_id=${COUNTY_ID}`],
      required: false,
      supportsDryRun: true,
      skip: SKIP_AUDITS,
    },
...
main().catch((error) => {
  console.error("Fatal Toledo market refresh error:", error instanceof Error ? error.message : error);
  process.exit(1);
});
