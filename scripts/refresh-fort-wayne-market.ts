#!/usr/bin/env tsx
/**
 * Repeatable Fort Wayne, IN market coverage refresh.
 *
 * Fort Wayne uses Allen County public parcels plus shared listing, contact,
...
const COUNTY_ID = 797481;
const FORT_WAYNE_ZIPS = [
  "46802", "46803", "46804", "46805", "46806", "46807", "46808", "46809",
...
async function main() {
  ...
  const resultPath = join(logDir, `fort-wayne-in-${stamp}.json`);
  const results: StepResult[] = [];

  await mkdir(logDir, { recursive: true });

  console.log("MXRE Fort Wayne, IN market refresh");
  ...
  const steps: Step[] = [
    {
      name: "Ingest Allen County public parcels",
      command: ["npx", "tsx", "scripts/ingest-in-parcels.ts", "--county=ALLEN"],
      required: true,
    ...
    {
      name: "Refresh Fort Wayne Redfin listing signals",
      command: ["npx", "tsx", "scripts/ingest-listings-fast.ts", "--state", "IN", "--zips", FORT_WAYNE_ZIPS, "--concurrency", dryLimit("3", "1"), ...(DRY_RUN ? ["--dry-run", "--skip-match", "--allow-partial"] : ["--skip-match", "--allow-partial"])],
      required: false,
      supportsDryRun: true,
      skip: SKIP_LISTINGS,
    ...
    {
      name: "Link Fort Wayne active listings",
      command: ["npx", "tsx", "scripts/link-market-listings-fast.ts", "--state=IN", "--city=Fort Wayne", `--county_id=${COUNTY_ID}`, "--create-shells", ...(DRY_RUN ? ["--dry-run"] : [])],
      required: false,
      supportsDryRun: true,
      skip: SKIP_LISTINGS,
    ...
    {
      name: "Enrich Fort Wayne Redfin listing detail pages",
      command: ["npx", "tsx", "scripts/enrich-redfin-detail-pages.ts", "--state=IN", "--city=Fort Wayne", `--limit=${dryLimit("5000", "25")}`, `--delay-ms=${dryLimit("300", "100")}`, ...(DRY_RUN ? ["--dry-run"] : [])],
      required: false,
      supportsDryRun: true,
      skip: SKIP_LISTINGS,
    ...
    {
      name: "Classify Fort Wayne active parcel assets",
      command: ["npx", "tsx", "scripts/classify-market-assets.ts", "--state=IN", "--city=Fort Wayne", `--county_id=${COUNTY_ID}`, "--active-listings-only", `--batch-size=${dryLimit("1500", "250")}`, ...(DRY_RUN ? ["--dry-run"] : [])],
      required: false,
      supportsDryRun: true,
      skip: SKIP_CLASSIFY,
      timeoutMs: DRY_RUN ? 5 * 60_000 : 20 * 60_000,
    ...
    {
      name: "Normalize Fort Wayne listing agent contacts",
      command: ["npx", "tsx", "scripts/enrich-listing-agent-contacts.ts", "--state=IN", "--city=Fort Wayne", `--limit=${dryLimit("5000", "25")}`, ...(DRY_RUN ? ["--dry-run"] : [])],
      required: false,
      supportsDryRun: true,
      skip: SKIP_AGENT_CONTACTS,
    },
    {
      name: "Score Fort Wayne creative finance signals",
      command: ["npx", "tsx", "scripts/score-creative-finance-signals.ts", "--state=IN", "--city=Fort Wayne", `--limit=${dryLimit("5000", "25")}`, ...(DRY_RUN ? ["--dry-run"] : [])],
      required: false,
      supportsDryRun: true,
      skip: SKIP_CREATIVE,
    },
    {
      name: "Paid Fort Wayne Property Detail enrichment",
      command: [
    ...
    {
      name: "Audit Fort Wayne agent coverage",
      command: ["npx", "tsx", "scripts/audit-on-market-agent-coverage.ts", "--state=IN", "--city=Fort Wayne"],
      required: false,
      supportsDryRun: true,
      skip: SKIP_AUDITS,
    },
    {
      name: "Audit Fort Wayne readiness",
      command: ["npx", "tsx", "scripts/market-readiness-summary.ts", "--state=IN", "--city=Fort Wayne", `--county_id=${COUNTY_ID}`],
      required: false,
      supportsDryRun: true,
      skip: SKIP_AUDITS,
    },
  ...
  console.log("\nFort Wayne market refresh summary");
  console.log("=".repeat(48));
...
main().catch((error) => {
  console.error("Fatal Fort Wayne market refresh error:", error instanceof Error ? error.message : error);
  process.exit(1);
});
